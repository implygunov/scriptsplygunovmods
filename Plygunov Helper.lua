...
script_name("Plygunov Helper")
script_description('Мульти-платформенный Lua скрипт-хелпер для игроков проекта Arizona Online')
script_author("PLYGUNOV MODS")
script_version("BETA 3.3 VIP")

print('Скрипт инициализирован. Версия: ' .. thisScript().version)
...
-------------------------------------------- JSON SETTINGS -------------------------------------------
local configDirectory = getWorkingDirectory():gsub('\\','/') .. "/Plygunov Helper"
local settings = {}
...
local modules = {
	commands = {
		...
		data = {
			commands = {
				my = {
					{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на часы с гравировкой Plygunov Helper и посмотрел{sex} время&/time&/do На часах видно время {get_time}.' , arg = '' , enable = true, waiting = '1.5' , bind = "{}" },
				...
				police = {
					{ cmd = '55', description = 'Проведение 10-55', text = '/r {my_doklad_nick} на CONTROL. Провожу 10-55 в районе {get_area} ({get_square}), СODE 4.&/m Водитель{get_storecar_model}, внимание!&/m Немедленно снизьте скорость и прижмитесь к обочине!&/m После остановки заглушите двигатель, держите руки на руле и не выходите из транспорта.&/m В случае неподчинения вы будете объявлены в розыск, и по вам будет открыт огонь!', arg = '', enable = true, waiting = '1.5', bind = "[101]" },
					{ cmd = '66', description = 'Проведение 10-66', text = '/r {my_doklad_nick} на CONTROL. Провожу 10-66 в районе {get_area} ({get_square}), СODE 3!&/m Водитель{get_storecar_model}, внимание!&/m Немедленно снизьте скорость и прижмитесь к обочине!&/m В случае неподчинения вы будете объявлены в розыск, и по вам будет открыт огонь!', arg = '', enable = true, waiting = '1.5', bind = "[102]" },
					{ cmd = 'zd' , description = 'Приветствие игрока' , text = 'Здравствуйте {get_ru_nick({arg_id})}&Я {my_ru_nick} - {fraction_rank} {fraction_tag}&Чем я могу Вам помочь?', arg = '{arg_id}' , enable = true , waiting = '1.5', bind = "{}"},
					...
					{ cmd = 'fara' , description = 'Оставить отпечаток на фаре' , text = '/me подойдя к транспортному средству, протёр левую фару, оставив отпечаток&/do Отпечаток успешно оставлен на левой фаре транспортного средства.', arg = '' , enable = true , waiting = '1.5', bind = "{}" },
					{ cmd = 'pas' , description = 'Запросить документы' ,  text = 'Здравствуйте, управление {fraction_tag}, я {fraction_rank} {my_ru_nick}&/do Cлева на груди жетон полицейского, справа именная нашивка с именем.&/me  достаёт своё удостоверение из кармана&/showbadge {arg_id}&Прошу предъявить документ, удостоверяющий вашу личность.&/n {get_nick({arg_id})}, введите /showpass {my_id}' , arg = '{arg_id}' , enable = true , waiting = '1.5' , bind = "{}", in_fastmenu = true},
					{ cmd = 'ts' , description = 'Выписать штраф' ,  text = '/do Бланк протокола и ручка находятся в нагрудном кармане.&/me достаёт из нагрудного кармана бланк протокола и ручку.&/me вписывает в бланк данные нарушителя&/writeticket {arg_id} {arg2}&/do Бланк протокола заполнен.&/me передаёт бланк со штрафом нарушителю для дальнейшей оплаты' , arg = '{arg_id} {arg2}' , enable = true, waiting = '1.5' , bind = "{}"  },
				...
				hospital = {	
					...
					{cmd = 'med' , description = 'Оформление игроку мед.карты' ,  text = 'Оформление мед. карты платное и зависит от её срока действия!&Мед. карта на 7 дней - ${price_med7}, на 14 дней - ${price_med14}.&Мед. карта на 30 дней - ${price_med30}, на 60 дней - ${price_med60}.&Скажите, вам на какой срок оформить мед. карту?&{show_medcard_menu}&Хорошо, тогда приступим к оформлению.&/me достаёт из своего мед.кейса пустую мед.карту, ручку и печать {fraction_tag}&/me открывает пустую мед.карту и начинает её заполнять, затем ставит печать {fraction_tag}&/me полностью заполнив мед.карту убирает ручку и печать обратно в свой мед.кейс&/todo Вот ваша мед.карта, берите*протягивая заполненную мед.карту человеку напротив себя&/medcard {arg_id} {get_medcard_status} {get_medcard_days} {get_medcard_price}&/n {get_nick({arg_id})}, примите предложение в /offer для получения медкарты' , arg = '{arg_id}' , enable = true, waiting = '1.5' , bind = "{}", in_fastmenu = true},
...
local checkbox = {
    ...
    auto_clicker_situation = imgui.new.bool(settings.general.auto_clicker_situation),
    ...
    auto_documentation = imgui.new.bool(settings.mj.auto_documentation),
	...
	if hotkey_no_errors and not isMode('') then
		...
		function updateHotkeyForCommand(command)
			...
			if command.arg == "" and command.bind ~= nil and command.bind ~= '{}' and command.bind ~= '[]' then
				...
				sampAddChatMessage('[Plygunov Helper] {ffffff}Создан хоткей для команды ' .. message_color_hex .. '/' .. command.cmd .. ' {ffffff}на клавишу '  .. message_color_hex .. getNameKeysFrom(command.bind), message_color)
			end
		end
...
function handleNewWeapon(weaponId)
    sampAddChatMessage('[Plygunov Helper] {ffffff}Обнаружено новое оружие с ID ' .. message_color_hex .. weaponId .. '{ffffff}, даю ему имя "оружие" и расположение "спина".', message_color)
    sampAddChatMessage('[Plygunov Helper] {ffffff}Изменить имя или расположение оружия вы можете в /helper - Главное меню - Режим RP отыгровки оружия - Настроить', message_color)
...
local binderTags = {
	...
	get_storecar_model = function ()
		...
		if closest_car then
			...
			sampAddChatMessage("[Plygunov Helper] {ffffff}Не удалось получить модель ближайшего т/c с водителем!", 0x009EFF)
		...
		if isCharInAnyCar(PLAYER_PED) then
			...
			if success and passengers and tonumber(passengers) > 0 then
				...
				if #my_passengers ~= 0 then
					...
					--sampAddChatMessage('[Plygunov Helper] В вашем авто нету ваших напарников!', -1)
					return 'Нету'
				end
			else
				return 'Нету'
			...
			--sampAddChatMessage('[Plygunov Helper] Вы не находитесь в авто, невозможно получить ваших напарников!', -1)
			return 'Нету'
		end
	end,
	switchCarSiren = function ()
		if isCharInAnyCar(PLAYER_PED) then
			...
			if getDriverOfCar(car) == PLAYER_PED then
				...
				--sampAddChatMessage('[Plygunov Helper] {ffffff}Вы не за рулём!', 0x009EFF)
				return (isCarSirenOn(car) and 'Выключи' or 'Врубай') .. ' мигалки!'
			end
		else
			--sampAddChatMessage('[Plygunov Helper] {ffffff}Вы не в автомобиле!', 0x009EFF)
			return "Кхм"
		end
	end,
	-- LC
...
function welcome_message()
	if not sampIsLocalPlayerSpawned() then 
		sampAddChatMessage('[Plygunov Helper] {ffffff}Инициализация хелпера прошла успешно!',message_color)
		sampAddChatMessage('[Plygunov Helper] {ffffff}Для полной загрузки хелпера сначало заспавнитесь (войдите на сервер)',message_color)
		repeat wait(0) until sampIsLocalPlayerSpawned()
	end
	sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка хелпера прошла успешно!', message_color)
	print('Полная загрузка хелпера прошла успешно!')
	-- show_arz_notify('info', 'Plygunov Helper', "Загрузка хелпера прошла успешно!", 3000)
	show_arz_sms("Привет, желаю тебе удачи!", "PLYGUNOV MODS", 815168, 120012, 6500, "https://mtgmods.github.io/arizona-helper/Resourse/logo.gif")

	if isMonetLoader() or settings.general.bind_mainmenu == nil then	
		sampAddChatMessage('[Plygunov Helper] {ffffff}Чтоб открыть меню хелпера введите команду ' .. message_color_hex .. '/helper', message_color)
	elseif hotkey_no_errors and settings.general.bind_mainmenu then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Чтоб открыть меню хелпера нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_mainmenu) .. ' {ffffff}или введите команду ' .. message_color_hex .. '/helper', message_color)
	else
		sampAddChatMessage('[Plygunov Helper] {ffffff}Чтоб открыть меню хелпера введите команду ' .. message_color_hex .. '/helper', message_color)
	end
end
		...
		if not isActiveCommand then
			...
			if cmd_arg == '{arg}' then
				if arg and arg ~= '' then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [аргумент]', message_color)
					playNotifySound()
				end
			elseif cmd_arg == '{arg_id}' then
				if isParamSampID(arg) then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока]', message_color)
					playNotifySound()
				end
			elseif cmd_arg == '{arg_id} {arg2}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
            ...
            elseif cmd_arg == '{arg_id} {arg2} {arg3}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 and arg3 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [число] [аргумент]', message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [число] [аргумент]', message_color)
			...
			elseif cmd_arg == '{arg_id} {arg2} {arg3} {arg4}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 and arg3 and arg4 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [число] [аргумент] [аргумент]', message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [число] [аргумент] [аргумент]', message_color)
				...
				lua_thread.create(function()
					...
					for line_index, line in ipairs(lines) do
						if command_stop then 
							...
							sampAddChatMessage('[Plygunov Helper] {ffffff}Отыгровка команды /' .. chat_cmd .. " успешно остановлена!", message_color) 
							break	
						else
							...
							elseif line == "{pause}" then
								sampAddChatMessage('[Plygunov Helper] {ffffff}Команда /' .. chat_cmd .. ' поставлена на паузу!', message_color)
								command_pause = true
								...
								if not command_stop then
									sampAddChatMessage('[Plygunov Helper] {ffffff}Продолжаю отыгровку команды /' .. chat_cmd, message_color)	
								end			
							else
								if not command_stop then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
			playNotifySound()
		end
	end)
end
function find_and_use_command(cmd, cmd_arg)
	...
	sampAddChatMessage('[Plygunov Helper] {ffffff}Не могу найти бинд для выполнения этой команды! Попробуйте сбросить настройки', message_color)
	playNotifySound()
end
function initialize_commands()
	...
	sampRegisterChatCommand("stop", function() 
		if isActiveCommand then 
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}В данный момент нету никакой активной команды/отыгровки!', message_color) 
		end
	end)
	sampRegisterChatCommand("debug", function() debug_mode = not debug_mode sampAddChatMessage('[Plygunov Helper] {ffffff}Отслеживание команд ' .. (debug_mode and 'включено!' or 'выключено!'), message_color) end)
	sampRegisterChatCommand("fixsize", function()
		settings.general.custom_dpi = 1.0
		settings.general.autofind_dpi = false
		sampAddChatMessage('[Plygunov Helper] {ffffff}Размер менюшек хелпера сброшен к стандартным! Перезапуск...', message_color)
		save_settings()
		reload_script = true
		thisScript():reload()
	end)
	...
	sampRegisterChatCommand('probiv', function(arg)
		if arg then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте /probiv [ID или Nick игрока]', message_color)
		end
	end)
		...
		sampRegisterChatCommand("mb", function(arg)
			if not isActiveCommand then
				if MembersWindow[0] then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Меню списка сотрудников закрыто!', message_color)
				else
					members_new = {} 
		...
		sampRegisterChatCommand("sob", function(arg)
			if not isActiveCommand then
				if isParamSampID(arg) then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/sob [ID игрока]', message_color)
					playNotifySound()
				end	
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
	...
	if isMode('police') or isMode('fbi') then
			...
			if not isActiveCommand then
				if isParamSampID(arg) then
					if #modules.smart_uk.data ~= 0 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Сначало загрузите/заполните систему умного розыска в /helper - Функции ' .. binderTags.fraction_tag(), message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/sum [ID игрока]', message_color)
					playNotifySound()
				end	
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
			...
			if not isActiveCommand then
				if isParamSampID(arg) then
					if #modules.smart_pdd.data ~= 0 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Сначало загрузите/заполните систему умных штрафов в /helper - Функции ' .. binderTags.fraction_tag(), message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/tsm [ID игрока]', message_color)
					playNotifySound()
				end	
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
		...
		sampRegisterChatCommand("afind", function (arg)
			if isParamSampID(arg) then
				...
				if afind then
					lua_thread.create(function ()
						sampAddChatMessage('[Plygunov Helper]{ffffff} Начинаю поиск игрока ' .. sampGetPlayerNickname(arg) .. '. Деактивация: ' .. message_color_hex .. '/afind', message_color)
						while afind do
					...
					sampAddChatMessage('[Plygunov Helper]{ffffff} Отключаю поиск игрока ' .. sampGetPlayerNickname(arg), message_color)
				end
			else
				sampAddChatMessage('[Plygunov Helper]{ffffff} Используйте ' .. message_color_hex .. '/afind [ID игрока]', message_color)
			end
		end)
		sampRegisterChatCommand("wanted", function(arg)
			sampSendChat('/wanted ' .. arg)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Лучше используйте /wanteds для автосканирования всего вантеда!', message_color)
		end)
		sampRegisterChatCommand("wanteds", function(arg)
			if WantedWindow[0] or updwanteds.stop then
				...
				sampAddChatMessage('[Plygunov Helper] {ffffff}Меню списка преступников закрыто!', message_color)
			elseif not isActiveCommand then
				lua_thread.create(function()
					local max_lvl = isMode('fbi') and 7 or 6
					sampAddChatMessage('[Plygunov Helper] {ffffff}Сканирование /wanted, ожидайте ' .. message_color_hex .. max_lvl .. ' {ffffff}секунд...', message_color)
					show_arz_notify('info', 'Plygunov Helper', "Сканирование /wanted...", 2500)
					wanted_new = {}
					check_wanted = true
					...
					if #wanted_new == 0 then
						sampAddChatMessage('[Plygunov Helper] {ffffff}Сейчас на сервере нету игроков с розыском!', message_color)
					else
						sampAddChatMessage('[Plygunov Helper] {ffffff}Сканирование /wanted окончено! Найдено преступников: ' .. #wanted_new, message_color)
						wanted = wanted_new
						...
						if settings.mj.awanted then
							...
							sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Функция AWANTED работает, вы можете закрыть менюшку списка преступников.', message_color)
							sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Если возле вас будет игрок с розыском - вы получите оповещение!', message_color)
						end
					end
				end)
			else
		...
		sampRegisterChatCommand("patrool", function(arg)
			if not isActiveCommand then
				...
				-- else
				-- 	sampAddChatMessage('[Plygunov Helper] {ffffff}Нельзя начать патруль, вы должны быть за рулём транспорта!', message_color)
				-- end
				PatroolMenu[0] = not PatroolMenu[0]
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
			...
			if not isActiveCommand then
				if isParamSampID(arg) then
					if #modules.smart_rptp.data ~= 0 then
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Сначало загрузите/заполните систему умного срока в /helper - Функции ' .. binderTags.fraction_tag(), message_color)
						playNotifySound()
					end
				else
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/pum [ID игрока]', message_color)
					playNotifySound()
				end	
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
		...
		sampRegisterChatCommand("spcar", function()
			if not isActiveCommand then
				lua_thread.create(function()
					isActiveCommand = true
					if isMonetLoader() and settings.general.mobile_stop_button then
						sampAddChatMessage('[Plygunov Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите кнопку внизу экрана', message_color)
						CommandStopWindow[0] = true
					elseif not isMonetLoader() and hotkey_no_errors and settings.general.bind_command_stop then
						sampAddChatMessage('[Plygunov Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_command_stop), message_color)
					else
						sampAddChatMessage('[Plygunov Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop', message_color)
					end
					...
					if command_stop then 
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Отыгровка команды /spcar успешно остановлена!', message_color) 
						return
					end
...
function show_fast_menu(id)
	if isParamSampID(id) then 
		sampAddChatMessage('[Plygunov Helper] {ffffff}Добавить/убрать команду из FastMenu можно в /helper - RP команды - Изменение, аргумент {arg_id}, галочка', message_color)
		player_id = tonumber(id)
		FastMenu[0] = true
	else
		if isMonetLoader() or settings.general.bind_fastmenu == nil then
			if not FastMenuPlayers[0] then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/hm [ID]', message_color)
			end
		elseif settings.general.bind_fastmenu and hotkey_no_errors then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/hm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/hm [ID]', message_color)
		end 
		playNotifySound()
...
function show_leader_fast_menu(id)
	if isParamSampID(id) then
		...
		if isMonetLoader() or settings.general.bind_leader_fastmenu == nil then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/lm [ID]', message_color)
		elseif settings.general.bind_leader_fastmenu and hotkey_no_errors then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/lm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_leader_fastmenu), message_color) 
		else
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. '/lm [ID]', message_color)
		end 
		playNotifySound()
...
function fast_heal_in_chat(id)
	if isMonetLoader() then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Чтоб вылечить игрока ' .. sampGetPlayerNickname(id) .. ', в течении 5-ти секунд нажмите кнопку',message_color)
		...
		lua_thread.create(function()
			...
			if heal_in_chat.bool then
				...
				heal_in_chat.bool = false
				sampAddChatMessage('[Plygunov Helper] {ffffff}Вы не успели вылечить игрока ' .. sampGetPlayerNickname(id), message_color)
			end
		end)
	elseif hotkey_no_errors then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Чтобы вылечить игрока ' .. sampGetPlayerNickname(id) .. ' нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_action) .. ' {ffffff}в течении 5-ти секунд!',message_color)
		show_arz_notify('info', 'Plygunov Helper', 'Нажмите ' .. getNameKeysFrom(settings.general.bind_action) .. ' чтобы быстро вылечить игрока', 5000)
		heal_in_chat.player_id = id
		heal_in_chat.bool = true
		lua_thread.create(function()
			wait(5000)
...
function sampGetPlayerIdByNickname(nick)
	...
	if id == nil then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка: не удалось получить ID игрока!', message_color)
		id = ''
	end
	return id
end
	...
	if doesFileExist(modules.arz_veh.path) and modules.arz_veh.data and #modules.arz_veh.data ~= 0 then
		...
		for _, vehicle in ipairs(modules.arz_veh.data) do
			if vehicle.model_id == id then
				check = true
				--sampAddChatMessage("[Plygunov Helper] {ffffff}Самый ближайший транспорт к вам это " .. vehicle.name ..  " [ID " .. id .. "].", message_color)
				return " " .. vehicle.name
			end
		end
		if not check then
		...
		sampAddChatMessage('[Plygunov Helper] {ffffff}Не удалось получить модель т/c с ID ' .. id .. ", так как отсуствует файл с моделями т/c", message_color)
		need_download_arzveh = true
	end
	if need_download_arzveh then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Пытаюсь скачать файл Vehicles.json в папку ' .. modules.arz_veh.path, message_color)
...
function downloadFileFromUrlToPath(url, path)
	...
	local function on_finish_download()
		if download_file == 'update' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Информация про наличие обновлений получена, проверка..',  message_color)
			local function check_need_update(updateInfo)
				...
				if uVer and thisScript().version ~= uVer then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Доступно обновление!', message_color)
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Обновление не нужно, у вас актуальная версия!', message_color)
				end
			end
		...
		elseif download_file == 'helper' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка новой версии хелпера успешно завершена! Перезагрузка..',  message_color)
			reload_script = true
			thisScript():unload()
		elseif download_file == 'smart_uk' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка системы умной выдачи розыска для сервера ' .. message_color_hex .. getARZServerName(getARZServerNumber()) .. ' [' .. getARZServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/sum [ID игрока]', message_color)
			MainWindow[0] = false
			load_module('smart_uk')
		elseif download_file == 'smart_pdd' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка системы умной выдачи штрафов для сервера ' .. message_color_hex .. getARZServerName(getARZServerNumber()) .. ' [' .. getARZServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/tsm [ID игрока]', message_color)
			MainWindow[0] = false
			load_module('smart_pdd')
		elseif download_file == 'smart_rptp' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка системы умного срока для сервера ' .. message_color_hex .. getARZServerName(getARZServerNumber()) .. ' [' .. getARZServerNumber() ..  '] {ffffff}завершена успешно!',  message_color)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Теперь вы можете использовать команду ' .. message_color_hex .. '/pum [ID игрока]', message_color)
			MainWindow[0] = false
			load_module('smart_rptp')
		elseif ddownload_file == 'arz_veh' then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузка всех кастомных т/с аризоны успешно заверешена!',  message_color)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Повторно используйте команду для определения модели т/c.',  message_color)
			load_module('arz_veh')
	...
	if isMonetLoader() then
		...
		downloadToFile(url, path, function(type, pos, total_size)
			...
			elseif type == "error" then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка загрузки: ' .. pos,  message_color)
			end
		end)
	else
...
function check_update()
	...
	sampAddChatMessage('[Plygunov Helper] {ffffff}Начинаю проверку на наличие обновлений...', message_color)
...
function check_resourses()
	if not doesFileExist(configDirectory .. '/Resourse/logo1.png') or not doesFileExist(configDirectory .. '/Resourse/logo2.png') then
		...
		print('Подгружаю логотип хелпера (файл .png)...')
	...
	if not doesFileExist(configDirectory .. "/Resourse/notify.mp3") then
		print('Подгружаю звук для оповещений хелпера (файл .mp3)...')
		download_file = 'notify'
		downloadFileFromUrlToPath('https://raw.githubusercontent.com/PLYGUNOVMODS/arizona-helper/main/Resourse/notify.mp3', configDirectory .. "/Resourse_notify.mp3")
	end
	if not doesFileExist(modules.arz_veh.path) then
		print('Подгружаю список т/c аризоны для опредленения моделей...')
		download_file = 'arz_veh'
		downloadFileFromUrlToPath('https://github.com/MTGMODS/arizona-helper/raw/refs/heads/main/SmartVEH/Vehicles.json', modules.arz_veh.path)
	end
end
...
function deleteHelperData(checker)
	...
	if checker == 'full' then
		os.remove(getWorkingDirectory():gsub('\\','/') .. "/Plygunov Helper.lua")
...
function check(id, message)
    if (message:rlower():find('авто') or message:rlower():find('кар') or message:rlower():find('машину')) and not message:rlower():find('карт') then
        get_lic_time(message)
        give_license(id, 'авто')
    end
    if message:rlower():find('мото') or message:rlower():find('моцик') then
        get_lic_time(message)
        give_license(id, 'мото')
    end
        ...
        function(response)
            if response.status_code == 200 then
                ...
                if ok and result then
                    sampAddChatMessage('[Plygunov Helper] {ffffff}Игрок найден.', message_color)
					probiv = result
                else
                    sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка декодирования ответа.', message_color)
                end
            elseif response.status_code == 422 then
                sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка 422: Никнейм не найден или введен с ошибкой.', message_color)
            elseif response.status_code == 401 then
                sampAddChatMessage('[Plygunov Helper] {ffffff}Неверный API ключ.', message_color)
            elseif response.status_code == 429 then
                sampAddChatMessage('[Plygunov Helper] {ffffff}Превышен лимит запросов. Повторите чуть позже.', message_color)
            else
                sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка API: ' .. tostring(response.status_code), message_color)
            end
        end,
        function(err)
            sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка запроса: ' .. tostring(err), message_color)
        end
    )
end
...
function sampev.onShowTextDraw(id, data)
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~r~Sport!') then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Активирован режим езды Sport!', message_color)
		return false
	end
	if data.text:find('~n~~n~~n~~n~~n~~n~~n~~n~~w~Style: ~g~Comfort!') then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Активирован режим езды Comfort!', message_color)
		return false
	end

	--sampAddChatMessage(data.text .. ' , ' .. data.modelId, -1)
	if settings.general.auto_repair and data.text:find('ЊPO‚PECC_PEMOHЏA') then
		lua_thread.create(function ()
			wait(50)
			sampAddChatMessage('[AS Helper] {ffffff}Авто-ремонт дорожного знака... (если есть ошибки, отключите данную функцию)', message_color)
			local textdraws_repair = {2080, 2105, 2104, 2103, 2102, 2101, 2100, 2099, 2098, 2081, 2105, 2104, 2103, 2102, 2101, 2100, 2099, 2098, 2082, 2105, 2104, 2103, 2102, 2101, 2100, 2099, 2098, 2083, 2105, 2104, 2103, 2102, 2101, 2100, 2099, 2098, 2084, 2105, 2104, 2103, 2102, 2101, 2100, 2099, 2098}
			for index, id in ipairs(textdraws_repair) do
				--sampAddChatMessage('click ' .. id, -1)
				sampSendClickTextdraw(id)
				wait(50 + (tonumber(sampGetPlayerPing(select(2,sampGetPlayerIdByCharHandle(PLAYER_PED))))))
			end
		end)
	end

	if data.text:find('hit') and asdebug then
		table.insert(as_debug.show_hit, id)
	end
...
function sampev.onSendTakeDamage(playerId,damage,weapon)
	if playerId ~= 65535 then
		...
		if isParamSampID(playerId) and playerId1 ~= playerId2 and tonumber(playerId) ~= 0 and weapon then
			...
			if weapon_name then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Игрок ' .. sampGetPlayerNickname(playerId) .. '[' .. playerId .. '] напал на вас используя ' .. weapon_name .. '['.. weapon .. ']!', message_color)
				if isMode('police') or isMode('fbi') or isMode('army') or isMode('prison') then
					if ((PatroolMenu[0] or PostMenu[0]) and (ComboPatroolCode[0] ~= 1)) then
						sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Ваш ситуационный код изменён на CODE 0.', message_color)
...
function sampev.onSendGiveDamage(playerId, damage, weapon, bodypart)
	if playerId ~= 65535 then
		...
		if (nick:find('Bogdan_Martelli') and getARZServerNumber():find('20')) or nick:find('%[20%]Bogdan_Martelli') then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Bogdan_Martelli - это разработчик Plygunov Helper!', message_color)
			sampAddChatMessage('[Plygunov Helper] {ffffff}Не нужно наносить урон разработчику хелпера, АСТАНАВИТЕСЬ :sob: :sob: :sob:', message_color)
			playNotifySound()
		end
	end
end
function sampev.onServerMessage(color,text)
	...
	if text:find("Номера телефонов государственных служб:") then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Номера телефонов государственных служб:', message_color)
		sampAddChatMessage('[Plygunov Helper] {ffffff}111 Баланс | 60 Время | 911 МЮ | 912 МЗ | 913 Такси | 914 Мехи | 8828 Банк | 997 Дома', message_color)
		return false
	end

	if (text:find('@' .. binderTags.my_id()) or text:find('@' .. binderTags.my_nick())) then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Кто-то упомянул вас в чате!', message_color)
		playNotifySound()
	end

	if text:find("Вы успешно надели маску") then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Вы надели маску', message_color)
		return false
	elseif text:find("Вы успешно выкинули маску") or text:find("Вы сняли маску") then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Вы сняли маску!', message_color)
		return false
	elseif text:find('Время действия маски (%d+) минут, после исхода времени ее придётся выбросить.') then
		...
		if (settings.general.auto_mask) then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Время действия маски ' .. min .. ' минут, после исхода времени автоматически надену новую!', message_color)
		else
			sampAddChatMessage('[Plygunov Helper] {ffffff}Время действия маски истекло, вам пришлось ее выбросить.', message_color)
		end
		return false
	elseif text:find('Время действия маски истекло, вам пришлось ее выбросить.') then
		if (settings.general.auto_mask) then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Время действия маски истекло, автоматически надеваю новую.', message_color)
			sampSendChat("/mask")
		else
			sampAddChatMessage('[Plygunov Helper] {ffffff}Время действия маски истекло, вам пришлось ее выбросить.', message_color)
		end
	...
	if (settings.general.auto_uval and settings.player_info.fraction_rank_number >= 9) then
		...
		if text:find("(.+) заглушил%(а%) игрока (.+) на 1 минут. Причина: %[AutoUval%] Ожидайте...") and auto_uval_checker then
			...
			if Name == MyName then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Увольняю игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
				auto_uval_checker = false
				temp = PlayerID .. ' ПСЖ'
				find_and_use_command("/uninvite {arg_id} {arg2}", temp)
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Другой заместитель/лидер уже увольняет игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
				auto_uval_checker = false
			end
		end
	end
	...
	if isMode('police') or isMode('fbi') then
		if settings.player_info.fraction_rank_number >= (isMode('fbi') and 4 or 5) then
			if text:find("%[(.-)%] (.-) (.-)%[(.-)%]: (.+)") and color == 766526463 then -- /f /fb или /r /rb без тега 
				...
				if message:find('Прошу обьявить в розыск (%d) степени дело N(%d+)%. Причина%: (.+)') and playerID ~= binderTags.my_id() then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте /givefsu ' .. playerID .. ' чтобы выдать розыск по запросу офицера ' .. name, message_color)
				end
			elseif text:find("%[(.-)%] %[(.-)%] (.+) (.-)%[(.-)%]: (.+)") and color == 766526463 then -- /r или /f с тегом
				local tag, tag2, rank, name, playerID, message = string.match(text, "%[(.-)%] %[(.-)%] (.+) (.-)%[(.-)%]: (.+)")
				if message:find('Прошу обьявить в розыск (%d) степени дело N(%d+)%. Причина%: (.+)') and playerID ~= binderTags.my_id() then
		...
		if text:find('Вы посадили игрока (.+) в тюрьму на (%d+) минут') and settings.general.auto_doklad_arrest then
			...
			sampSendChat('/r ' .. binderTags.my_doklad_nick() .. ' на CONTROL. Преступник ' .. nick:gsub('_', ' ') .. ' посажен в КПЗ на ' .. mins .. ' минут!')
		...
		if text:find(binderTags.my_nick() .. ' обыскивает (.+)') and settings.general.auto_time then
			lua_thread.create(function ()
				wait(500)
				sampSendChat('/time')
			end)
		end
		if text:find("розыск%! Обвинитель%: " .. binderTags.my_nick()) and settings.general.auto_time then
			lua_thread.create(function ()
				wait(500)
				sampSendChat('/time')
			end)
		end
		if text:find('{ffffff}Вы подобрали обломок, теперь вам нужно отнести его и {ff0000}положить в общую кучу') then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Вы подобрали завал, теперь вам нужно отнести его в общую кучу!', message_color)
			if true then
				sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Запускаю отыгровку вашего действия в чат...', message_color)
				sampSendChat('/me наклонившись к завалу, крепко схватил' .. binderTags.sex() .. ' его и поднял' .. binderTags.sex() .. ' его с земли')
			end
			return false
		end
		if text:find('{ffffff}Вы положили обломок в общую кучу, отправляйтесь к следующему завалу.') then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Вы положили завал в общую кучу, теперь отправляйтесь к следующему завалу.', message_color)
			if true then
				sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Запускаю отыгровку вашего действия в чат...', message_color)
				sampSendChat('/me наклоняется к общей куче завалов и бросает туда завал со своих рук')
			end
			return false
		end
	end
	...
	if isMode('hospital') then
				...
				if (nick and id and message and tonumber(id) ~= select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) then
					for pon, keyword in ipairs(heal_in_chat.worlds) do
						if (message:rupper():find(keyword:rupper())) then
							...
							elseif settings.mh.auto_heal then
								lua_thread.create(function ()
									wait(50)
									sampAddChatMessage('[Plygunov Helper] {ffffff}Автоматический хил игрока ' .. sampGetPlayerNickname(id) .. " по просьбе: " .. keyword, message_color)
		...
		if text:find('%(%( Чтобы принять вызов, введите /godeath (%d+). Оплата за вызов (.+) %)%)') then
			...
			if godeath_locate == 'неизвестном' then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Из города ' .. message_color_hex .. godeath_city .. ' {ffffff}поступил вызов о пострадавшем ' .. message_color_hex .. sampGetPlayerNickname(godeath_player_id), message_color)
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Из города ' .. message_color_hex .. godeath_city .. ' (' .. godeath_locate .. ') {ffffff}поступил вызов о пострадавшем ' .. message_color_hex .. sampGetPlayerNickname(godeath_player_id), message_color)
			end
			sampAddChatMessage('[Plygunov Helper] {ffffff}Чтобы принять вызов, используйте команду ' .. message_color_hex .. cmd .. ' '.. godeath_player_id, message_color)
			return false
		end
		...
		if (auto_healme and text:find('Вы отправили предложение о лечении')) then
			--sampAddChatMessage('[Plygunov Helper] {ffffff}Самохил будет через 7 секунд, ожидайте...', message_color)
			sampSendChat('/offer')
			return false
		end
		...
		if text:find("Пациент (.+) вызывает врачей (.+)холл(.+)этаж") then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Пациент ' .. nick .. ' вызывает врача в холл больницы!', message_color)
			return false
		end
	end	
	...
	if isMode('lc') then
		...
		if (text:find('(.+)%[(%d+)%] говорит:{B7AFAF} (.+)')) and settings.general.auto_lic then
			local nick, id, message = text:match('(.+)%[(%d+)%] говорит:{B7AFAF} (.+)')
			if (nick and id and message and tonumber(id) ~= select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) then
				lua_thread.create(function ()
					wait(50)
		...
		if (text:find('(.+)%[(%d+)%] кричит: (.+)')) and settings.general.auto_lic then
			local nick, id, message = text:match('(.+)%[(%d+)%] кричит: (.+)')
			if (nick and id and message and tonumber(id) ~= binderTags.my_id()) then
				lua_thread.create(function ()
					wait(50)
					check(id, message)
				end)
	...
	if (text:find('Bogdan_Martelli') and getARZServerNumber():find('20')) or text:find('%[20%]Bogdan_Martelli') then
		...
		if text:find('%[20%]Bogdan_Martelli%[%d+%]') then
			...
			text = string.gsub(text, '%[20%]Bogdan_Martelli%[%d+%]', message_color_hex .. '[20]PLYGUNOVMODS[' .. id .. ']' .. lastColor)
		elseif text:find('%[20%]Bogdan_Martelli') then
			text = string.gsub(text, '%[20%]Bogdan_Martelli', message_color_hex .. '[20]PLYGUNOVMODS' .. lastColor)
		elseif text:find('Bogdan_Martelli%[%d+%]') then
			local id = text:match('Bogdan_Martelli%[(%d+)%]') or ''
			text = string.gsub(text, 'Bogdan_Martelli%[%d+%]', message_color_hex .. 'PLYGUNOVMODS[' .. id .. ']' .. lastColor)
		elseif text:find('Bogdan_Martelli') then
			text = string.gsub(text, 'Bogdan_Martelli', message_color_hex .. 'PLYGUNOVMODS' .. lastColor)
		end
		return {color,text}
	end
end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if title:find('Основная статистика') and check_stats then
		if text:find("{FFFFFF}Имя: {B83434}%[(.-)]") then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Ваше имя и фамилия обнаружены: ' .. settings.player_info.name_surname, message_color)
		end
		if text:find("{FFFFFF}Пол: {B83434}%[(.-)]") then
			settings.player_info.sex = text:match("{FFFFFF}Пол: {B83434}%[(.-)]")
			sampAddChatMessage('[Plygunov Helper] {ffffff}Ваш пол обнаружен: ' .. settings.player_info.sex, message_color)
		end
		if text:find("{FFFFFF}Организация: {B83434}%[(.-)]") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Вы не состоите в организации!', message_color)
				settings.player_info.fraction_tag = "none"
				settings.general.fraction_mode = "none"
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Ваша организация обнаружена, это: '..settings.player_info.fraction, message_color)
				local fraction_data = {
				...
				if data then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Вашей организации присвоен тег '..settings.player_info.fraction_tag .. ". Но вы можете изменить его.", message_color)
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Ваша организация пока что не поддерживается в хелпере!', message_color)
					sampAddChatMessage('[Plygunov Helper] {ffffff}Пройдите ручную настройку хелпера для инициализации...', message_color)
				end
				if text:find("{FFFFFF}Должность: {B83434}(.+)%((%d+)%)") then
					...
					sampAddChatMessage('[Plygunov Helper] {ffffff}Ваша должность обнаружена, это: '..settings.player_info.fraction_rank.." ("..settings.player_info.fraction_rank_number..")", message_color)
					if settings.player_info.fraction_rank_number >= 9 then
					...
					settings.player_info.fraction_rank = "none"
					settings.player_info.fraction_rank_number = 0
					sampAddChatMessage('[Plygunov Helper] {ffffff}Не могу получить ваш ранг!',message_color)
				end
			end
		end
		save_settings()
	...
	if members_info.check and title:find('(.+)%(В сети: (%d+)%)') then
		...
		elseif #members_new ~= 0 then
			...
			--sampAddChatMessage('[Plygunov Helper] {ffffff}Сканирование /members окончено! Для закрытия меню введите команду ещё раз', message_color)
		else
			sampSendDialogResponse(dialogid, 0, 0, 0)
			sampAddChatMessage('[Plygunov Helper]{ffffff} Список сотрудников пуст!', message_color)
			members_info.check = false
        end
        return false
    end
	...
	if (isMode('hospital')) then
		...
		if text:find("Проверьте и подтвердите данные перед выдачей мед карты") and text:find("Полностью здоров") then  -- автовыдача мед.карты
			sampAddChatMessage('[Plygunov Helper] {ffffff}Ожидайте пока игрок подтвердит получение мед. карты', message_color)
			sampSendDialogResponse(dialogid, 1, 0, 0)
			return false
		end
	end
	...
	if isMode('police') or isMode('fbi') then
		if message:find(" (.+) достал скрепки для взлома наручников") then
			...
			if result then
				...
				local dist = getDistanceBetweenCoords3d(mx, my, mz, x, y, z)
				if dist <= 1.5 then
					sampAddChatMessage('[Plygunov Helper] {ffffff}Внимание! Игрок ' .. nick .. '[' .. sampGetPlayerIdByNickname(nick) .. '] использует скрепки и начинает взламывать наручники!', message_color)
					sampAddChatMessage('[Plygunov Helper] {ffffff}Пытаюсь изьять скрепки у этого игрока.', message_color)
					find_and_use_command('/bot {arg_id}', sampGetPlayerIdByNickname(nick))
				elseif dist > 50 then
					sampAddChatMessage('[Plygunov Helper] {ffffff}Внимание! Игрок ' .. nick .. '[' .. sampGetPlayerIdByNickname(nick) .. '] использует скрепки и начинает взламывать наручники!', message_color)
					sampAddChatMessage('[Plygunov Helper] {ffffff}Подойдите к игроку ' .. nick .. ' и используйте команду /bot ' .. sampGetPlayerIdByNickname(nick), message_color)
				end
			end
		end
	end
	...
	if id == 220 then
		...
		if isMonetLoader() then 
			...
			if cmd == 17 then
				...
				if cmd:find('"businessCenter","Проверка документов","Найдите документы в конвертах"') then
					local find = cmd:match('"Найдите документы в конвертах",%[(.+)%]%]')
					sampAddChatMessage("[Plygunov Helper - Ассистент] {ffffff}Правильные конверты: " .. find:gsub(',', ' '), message_color)
				end
			end
		end
...
function give_rank()
			...
			lua_thread.create(function()
				...
				for _, line in ipairs(lines) do 
					if command_stop then 
						...
						sampAddChatMessage('[Plygunov Helper] {ffffff}Отыгровка команды /' .. command.cmd .. " успешно остановлена!", message_color) 
						return 
					end
					if wait_tag then
						for tag, replacement in pairs(binderTags) do
	...
	if not command_find then
		sampAddChatMessage('[Plygunov Helper] {ffffff}Бинд для изменения ранга отсутствует либо отключён!', message_color)
		sampAddChatMessage('[Plygunov Helper] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
...
imgui.OnFrame(
    ...
    function(player)
        ...
        imgui.Begin(fa.GEARS .. u8' Первоначальная настройка Plygunov Helper ' .. fa.GEARS, InititalWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
        change_dpi()
		if init.step == 0 then
			...
			if imgui.CenterButton(fa.CIRCLE_ARROW_RIGHT .. u8(' Автоматически, исходя из вашей /stats (статистики) ') .. fa.CIRCLE_ARROW_LEFT) then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Пытаюсь получить ваш /stats поскольку остуствуют данные про вас!', message_color)
				check_stats = "reload"
				sampSendChat('/stats')
			...
			if imgui.CenterButton(fa.CIRCLE_ARROW_RIGHT .. u8(' Вручную пройти первоначальную настройку хелпера ') .. fa.CIRCLE_ARROW_LEFT) then
				init.step = 1
				sampAddChatMessage('[Plygunov Helper] {ffffff}Выберите свою организацию для инициализации нужных команд скрипта...', message_color)
			end
			imgui.Separator()
									...
									if imgui.BeginPopupModal(fa.PEN_TO_SQUARE .. u8(" Редактирование подпункта##") .. title .. chapter.name .. index .. chapter_index, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
										...
										if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить ' .. fa.FLOPPY_DISK, imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
											...
											if isValid then
												...
												sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка в указанных данных, исправьте!', message_color)
											end
										end
										imgui.EndPopup()
									end
...
function render_assist_items(name, config, description, description_hovered)
	...
	function render_fractions_functions()
		if imgui.BeginTabItem(fa.GEARS .. u8' Функции орги') then
			...
			-- if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 58 * settings.general.custom_dpi), true) then
				...
				-- 	-- 		settings.mj.auto_documentation = checkbox.autodocumentation[0]
				-- 	-- 		save_settings()
				-- 	-- 	end
				...
				-- 	-- 		settings.general.auto_clicker_situation = checkbox.auto_clicker[0]
				-- 	-- 		save_settings()
				-- 	-- 	end
				...
				-- 				sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка, нельзя включить "Хил из чата" так как у вас нету Mimgui Hotkeys!', message_color)
				-- 			else
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(getHelperIcon() .. " Plygunov Helper " .. getHelperIcon() .. "##main", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		change_dpi()
		if imgui.BeginTabBar('чё код смотришь, а?') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 169 * settings.general.custom_dpi), true) then
					...
					if imgui.BeginPopupModal(getHelperIcon() .. u8' Смена организации ' .. getHelperIcon() .. '##fraction', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						if imgui.Button(fa.GEARS .. u8' Сбросить ' .. fa.GEARS .. '##reset_fraction', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							modules.commands.data.commands.my = {{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на свои часы с гравировкой Plygunov Helper и посмотрел{sex} время&/time&/do На часах видно время {get_time}.' , arg = '' , enable = true, waiting = '1.5' , bind = "{}" }}
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 28 * settings.general.custom_dpi), true) then
					if thisScript().version:find('VIP') then
						imgui.CenterText(fa.CROWN .. u8" Вы VIP пользователь данного хелпера! PLYGUNOV MODS благодарит вас за поддержку " .. fa.CROWN)
					else
						imgui.Columns(2)
						imgui.Text(fa.HAND_HOLDING_DOLLAR .. u8" Вы можете финансово поддержать автора скрипта (PLYGUNOV MODS) донатом " .. fa.HAND_HOLDING_DOLLAR)
						imgui.SetColumnWidth(-1, 480 * settings.general.custom_dpi)
						imgui.NextColumn()
						...
						if imgui.BeginPopupModal(fa.SACK_DOLLAR .. u8' Поддержка разработчика', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
							...
							imgui.CenterText(u8'Свяжитесь с PLYGUNOV MODS:')
							...
							if imgui.Button(u8('Telegram'), imgui.ImVec2(imgui.GetMiddleButtonX(2), 25 * settings.general.custom_dpi)) then
								openLink('https://t.me/PLYGUNOVMODS')
							end
							imgui.SameLine()
							...
							if imgui.BeginChild('##fastmenu'..name, imgui.ImVec2(194 * settings.general.custom_dpi, 339 * settings.general.custom_dpi), true) then
								...
								if imgui.Button(fa.GEAR .. u8(' Настроить команды меню ') .. '##' .. name) then
									if name == 'PieMenu' then
										sampAddChatMessage('[Plygunov Helper] {ffffff}PieMenu временно недоступен для редактирования', message_color)
									else
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 145 * settings.general.custom_dpi), true) then
					...
					imgui.Separator()
					imgui.Text(fa.CIRCLE_USER..u8" Разработчик данного хелпера: PLYGUNOV MODS")
					imgui.Separator()
					...
					imgui.Separator()
					imgui.Text(fa.GLOBE..u8" Другие скрипты от PLYGUNOV MODS:")
					imgui.SameLine()
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 60 * settings.general.custom_dpi), true) then
					...
					if monet_no_errors then
						...
						if imgui.CenterColumnRadioButtonIntPtr(u8" Сustom ", theme, 0) then
							...
							sampAddChatMessage('[Plygunov Helper] {ffffff}Установите библиотеку MoonMonet!', message_color)
						end
					end
					imgui.NextColumn()
					...
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ' .. fa.TRIANGLE_EXCLAMATION .. '##change_size', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						...
						if imgui.Button(fa.CIRCLE_ARROW_RIGHT .. u8' Да, изменить ' .. fa.CIRCLE_ARROW_LEFT .. "##change_size", imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							...
							if isMonetLoader() and new_dpi < MONET_DPI_SCALE then
								sampAddChatMessage('[Plygunov Helper] {ffffff}Для вашего дисплея нельзя сделать размер меньше ' .. MONET_DPI_SCALE, message_color)
								imgui.CloseCurrentPopup()
							else
								settings.general.custom_dpi = new_dpi
								save_settings()
								sampAddChatMessage('[Plygunov Helper] {ffffff}Перезагрузка скрипта для пременения размера окон...', message_color)
								reload_script = true
								thisScript():reload()
							end
						end
				...
				if imgui.BeginChild("##4",imgui.ImVec2(589 * settings.general.custom_dpi, 35 * settings.general.custom_dpi),true) then
					if imgui.Button(fa.POWER_OFF .. u8" Выключение хелпера " .. fa.POWER_OFF, imgui.ImVec2(imgui.GetMiddleButtonX(3), 25 * settings.general.custom_dpi)) then
						reload_script = true
						sampAddChatMessage('[Plygunov Helper] {ffffff}Хелпер приостановил свою работу до следущего входа в игру!', message_color)
						if not isMonetLoader() then 
							sampAddChatMessage('[Plygunov Helper] {ffffff}Либо используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы запустить хелпер.', message_color)
						end
						thisScript():unload()
					end
					imgui.SameLine()
					...
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ' .. fa.TRIANGLE_EXCLAMATION .. '##delete', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						change_dpi()
						imgui.CenterText(u8'Вы действительно хотите удалить Plygunov Helper?')
						...
						if imgui.Button(fa.TRASH_CAN .. u8' Да, удалить ' .. fa.TRASH_CAN .. '##delete_helper', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							...
							sampAddChatMessage('[Plygunov Helper] {ffffff}Хелпер полностью удалён из вашего устройства!', message_color)
							thisScript():unload()
						end
						imgui.End()
					end
    ...
    function(player)
		...
		if imgui.Button(fa.KEYBOARD .. u8' Забиндить ' .. fa.KEYBOARD  .. '##binder_bind', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			if ComboTags[0] == 0 then
				if isMonetLoader() then
					sampAddChatMessage('[Plygunov Helper] {ffffff}Данная функция доступа только на ПК!', message_color)
				else
					if hotkey_no_errors then
						imgui.OpenPopup(fa.KEYBOARD .. u8' Бинд для команды /' .. binder_data.change_cmd)
					else
						sampAddChatMessage('[Plygunov Helper] {ffffff}Данная функция недоступа, причина отсуствуют файлы библиотеки mimgui_hotkeys!', message_color)
					end
				end
			else
				sampAddChatMessage('[Plygunov Helper] {ffffff}Данная функция доступа только если команда "Без аргументов"', message_color)
			end
		end
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
			...
			if ffi.string(input_cmd):find('%W') or ffi.string(input_cmd) == '' or ffi.string(input_description) == '' or ffi.string(input_text) == '' then
				...
				for _, command in ipairs(temp_array) do
					if command.cmd == binder_data.change_cmd and command.arg == binder_data.change_arg and command.text:gsub('&', '\n') == binder_data.change_text then
						...
						save_module('commands')
						if command.arg == '' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg}' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id}' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2}' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2} {arg3}' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [число] [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2} {arg3} {arg4}' then
							sampAddChatMessage('[Plygunov Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [число] [аргумент] [аргумент] {ffffff}успешно сохранена!', message_color)
						end
...
imgui.OnFrame(
    ...
    function(player)
		if #members == 0 then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Ошибка, список сотрудников пустой!', message_color)
			MembersWindow[0] = false
    ...
    function(player)
		...
		if settings.mj.auto_update_wanteds then
			...
			if imgui.Button(u8'Обновить список преступников', imgui.ImVec2(340 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				WantedWindow[0] = false
				sampAddChatMessage('[Plygunov Helper] {ffffff}Вы можете включить авто-обновление /wanteds в настройках Ассистента!', message_color)
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .. " Plygunov Helper##fast_meg_button", MegafonWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoBackground + imgui.WindowFlags.NoTitleBar)
		change_dpi()
		if not isMonetLoader() and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() then player.HideCursor = true else player.HideCursor = false end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(" Plygunov Helper##TaserWindow", TaserWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoBackground + imgui.WindowFlags.NoTitleBar)
		change_dpi()
		if not isMonetLoader() and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() then player.HideCursor = true else player.HideCursor = false end
...
imgui.OnFrame(
    ...
    function(player)
		...
		if modules.smart_uk.data ~= nil and isParamSampID(player_id) then
            ...
            sampAddChatMessage('[Plygunov Helper] {ffffff}Произошла ошибка умного розыска (нету данных либо игрок офнулся)!', message_color)
            SumMenuWindow[0] = false
        end
        imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
		...
		if modules.smart_rptp.data ~= nil and isParamSampID(player_id) then
            ...
            sampAddChatMessage('[Plygunov Helper] {ffffff}Произошла ошибка умного срока (нету данных либо игрок офнулся)!', message_color)
            SumMenuWindow[0] = false
        end
        imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
		...
		if modules.smart_pdd.data ~= nil and isParamSampID(player_id) then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Произошла ошибка умных штрафов (нету данных либо игрок офнулся)!', message_color)
            TsmMenuWindow[0] = false
        end
        imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
		...
		if not check then
			sampAddChatMessage('[Plygunov Helper] {ffffff}Нету команд включенных на работу в FastMenu!', message_color)
			FastMenu[0] = false
		end
		imgui.End()
    end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(getHelperIcon().." Plygunov Helper " .. getHelperIcon() .. "##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(getHelperIcon() .. " Plygunov Helper " .. getHelperIcon() .. "##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * settings.general.custom_dpi), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(getHelperIcon() .." Plygunov Helper " .. getHelperIcon() .. "##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		...
		if imgui.Button(fa.DOWNLOAD ..u8' Загрузить новую версию',  imgui.ImVec2(250 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
			if thisScript().version:find('VIP') then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Загрузите новую версию из VIP чата!', message_color)
			else
				download_file = 'helper'
				downloadFileFromUrlToPath(update.url, getWorkingDirectory():gsub('\\','/') .. "/Plygunov Helper.lua")
			end
			UpdateWindow[0] = false
		end
		imgui.End()
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			...
			sampAddChatMessage('[Plygunov Helper] {ffffff}Прозиошла ошибка, ID игрока недействителен!', message_color)
			SobesMenu[0] = false
		end
    end
)
				...
				if u8:decode(ffi.string(input)) == '' or value.name:rupper():find(u8:decode(ffi.string(input)):rupper()) or value.id == tonumber(u8:decode(ffi.string(input)))  then
					...
					if imgui.BeginPopupModal(fa.GUN .. u8' Название оружия ' .. fa.GUN .. '##weapon_name' .. index, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize ) then
						...
						if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить ' .. fa.FLOPPY_DISK, imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							value.name = u8:decode(ffi.string(input))
							save_module('rpgun')
							initialize_guns()
							imgui.StrCopy(input, '')
							imgui.CloseCurrentPopup()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HOSPITAL.." Plygunov Helper " .. fa.HOSPITAL .. "##medcard", MedCardMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
		...
		if imgui.Button(fa.ID_CARD_CLIP..u8" Выдать мед.карту", imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					...
					lua_thread.create(function()
						...
						for line_index, line in ipairs(lines) do 
							...
							if wait_tag then
								...
								if line == "{pause}" then
									sampAddChatMessage('[Plygunov Helper] {ffffff}Команда /' .. command.cmd .. ' поставлена на паузу!', message_color)
									command_pause = true
									CommandPauseWindow[0] = true
									while command_pause do
										wait(0)
									...
									if not command_stop then
										sampAddChatMessage('[Plygunov Helper] {ffffff}Продолжаю отыгровку команды /' .. command.cmd, message_color)	
									end					
								else
									sampSendChat(line)
									if debug_mode then sampAddChatMessage('[Plygunov Helper DEBUG] SEND: ' .. line, message_color) end	
			...
			if not command_find then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Бинд для выдачи мед.карты отсутствует либо отключён!', message_color)
				sampAddChatMessage('[Plygunov Helper] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
			end
			MedCardMenu[0] = false
		end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HOSPITAL.." Plygunov Helper " .. fa.HOSPITAL .. "##recept", ReceptMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
		...
		if imgui.Button(fa.CAPSULES..u8" Выдать рецепты" , imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
			...
			if not command_find then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Бинд для выдачи рецептов отсутствует либо отключён!', message_color)
				sampAddChatMessage('[Plygunov Helper] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
			end
			ReceptMenu[0] = false
		end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HOSPITAL.." Plygunov Helper " .. fa.HOSPITAL .. "##ant", AntibiotikMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
		...
		if imgui.Button(fa.CAPSULES..u8" Выдать антибиотики" , imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
			...
			if not command_find then
				sampAddChatMessage('[Plygunov Helper] {ffffff}Бинд для выдачи антибиотиков отсутствует либо отключён!', message_color)
				sampAddChatMessage('[Plygunov Helper] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
			end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HOSPITAL.." Plygunov Helper " .. fa.HOSPITAL .. "##fast_heal", FastHealMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar +  imgui.WindowFlags.AlwaysAutoResize )
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(getHelperIcon() .. u8" Plygunov Helper " .. getHelperIcon() .. '##patrool_info_menu', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
		change_dpi()
		if not isMonetLoader() and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() then player.HideCursor = true else player.HideCursor = false end
		if patrool.active then
			...
			if imgui.SmallButton(fa.GEAR) then
				imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' Plygunov Helper##patrool_select_code'))
			end
			imgui.Separator()
		...
		if imgui.BeginPopup(fa.BUILDING_SHIELD .. u8(' Plygunov Helper##patrool_select_code'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
			change_dpi()
			player.HideCursor = false 
			imgui.PushItemWidth(150 * settings.general.custom_dpi)
			if imgui.Combo('##patrool_code', ComboPatroolCode, ImItemsPatroolCode, #combo_patrool_code_list) then
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(settings.windows_pos.patrool_menu.x, settings.windows_pos.patrool_menu.y), imgui.Cond.FirstUseEver)
		--imgui.SetNextWindowSize(imgui.ImVec2(225 * settings.general.custom_dpi, 113 * settings.general.custom_dpi), imgui.Cond.FirstUseEver)
		imgui.Begin(getHelperIcon() .. u8" Plygunov Helper " .. getHelperIcon() .. '##post_info_menu', PostMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize )
		change_dpi()
		if not isMonetLoader() and not sampIsChatInputActive() and not sampIsDialogActive() and not isSampfuncsConsoleActive() then player.HideCursor = true else player.HideCursor = false end
		if patrool.active then
			...
			imgui.SameLine()
			if imgui.SmallButton(fa.GEAR) then
				imgui.OpenPopup(fa.BUILDING_SHIELD .. u8(' Plygunov Helper##post_select_code'))
			end
			imgui.Separator()
		...
		--imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
		if imgui.BeginPopup(fa.BUILDING_SHIELD .. u8(' Plygunov Helper##post_select_code'), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
			change_dpi()
			player.HideCursor = false 
			imgui.PushItemWidth(150 * settings.general.custom_dpi)
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(getHelperIcon() .." Plygunov Helper " .. getHelperIcon() .. '##probiv', ProbivMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		change_dpi()
		...
		if probiv ~= nil then
			if imgui.BeginChild('##probiv_2', imgui.ImVec2(400 * settings.general.custom_dpi, 405 * settings.general.custom_dpi), true) then
				...
				imgui.Text(fa.TRIANGLE_EXCLAMATION .. u8(' Варны: ') .. ((probiv.warnings .. "/3") or "nil"))
				imgui.Separator()
			...
			if imgui.BeginChild('##probiv_1', imgui.ImVec2(300 * settings.general.custom_dpi, 205 * settings.general.custom_dpi), true) then
				...
				if imgui.CenterButton(fa.CIRCLE_ARROW_RIGHT .. u8(' Узнать инфу об игроке ') .. fa.CIRCLE_ARROW_LEFT) then
					sampAddChatMessage('[Plygunov Helper] {ffffff}Получение информации про игрока ' .. ffi.string(input) .. "...", message_color)
...
function main()
	...
	while true do
			...
			if patrool.active and isCharInAnyCar(PLAYER_PED) and settings.general.auto_change_code_siren then
				...
				if currentSirenState ~= lastSirenState then
					...
					if currentSirenState then
						sampAddChatMessage("[Plygunov Helper - Ассистент] {ffffff}В вашем т/с была включена сирена, изменяю ситуационный код на CODE 3!", message_color)
						ComboPatroolCode[0] = 4
						patrool.code = combo_patrool_code_list[ComboPatroolCode[0] + 1]
					else
						sampAddChatMessage("[Plygunov Helper - Ассистент] {ffffff}В вашем т/с была отключена сирена, изменяю ситуационный код на CODE 4.", message_color)
						ComboPatroolCode[0] = 5
						patrool.code = combo_patrool_code_list[ComboPatroolCode[0] + 1]
					end
				end
				...
				if #wanted ~= 0 then
					for i, v in ipairs(wanted) do
						...
						if sampIsPlayerConnected(id) and _ then
							...
							sampAddChatMessage('[Plygunov Helper - Ассистент] {ffffff}Недалеко от вас обнаружен(-а) ' .. v.nick .. " с " .. v.lvl .. '-м уровнем розыска!', message_color)
							show_arz_notify('info', 'Plygunov Helper', 'Обнаружен игрок в розыске недалеко от вас!', 2500)
							playNotifySound()
		...
		if (settings.general.auto_update_members and MembersWindow[0]) then
			if (updmembers.check) then
				updmembers.time = os.difftime(os.time(), updmembers.last_time)
				if tonumber(updmembers.time) > 8.5 then
					updmembers.time = 0
					updmembers.last_time = os.time()
					updmembers.check = false
				end
			elseif (not updmembers.check) then 
				members_new = {}
				members_info.check = true
				sampSendChat("/members")
				updmembers.check = true
			end
		end
		...
		if (settings.general.auto_notify_payday) then
			...
			if ((currentMinute == "55" or currentMinute == "25") and currentSecond == "00") then
				if sampGetPlayerColor(binderTags.my_id()) == 368966908 then
					sampAddChatMessage('[Plygunov Helper] {ffffff}Через 5 минут будет PAYDAY. Наденьте форму чтобы не пропустить зарплату!', message_color)
					playNotifySound()
					wait(1000)
				end
			end
...
function InitCrashMessage()
	local file_path =  getWorkingDirectory():gsub('\\','/') .. "/.Plygunov Helper Crash Message.lua"
	if not doesFileExist(file_path) then
		local content = [[
function onSystemMessage(msg, type, script)
    if type == 3 and script and script.name == 'Plygunov Helper' and msg and not msg:find('Script died due to an error') then
        local errorMessage = ('{ffffff}Произошла непредусмотренная ошибка в работе скрипта, из-за чего он был отключён!\n\n' ..
		'Отправьте скриншот этого сообщения в {ff9900}тех.поддержку PLYGUNOV MODS (Telegram/Discord/BlastHack){ffffff}.\n\n' ..
		'Детали возникшей ошибки:\n{ff6666}' .. msg)
        sampShowDialog(789789, '{009EFF}Plygunov Helper [' .. script.version .. ']', errorMessage, '{009EFF}Закрыть', '', 0)
    end
end]]
...
function onScriptTerminate(script, game_quit)
    if script == thisScript() and not game_quit and not reload_script then
		...
		playNotifySound()
		sampAddChatMessage('[Plygunov Helper] {ffffff}Произошла неизвестная ошибка, хелпер приостановил свою работу!', message_color)
		if not isMonetLoader() then 
			sampAddChatMessage('[Plygunov Helper] {ffffff}Используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы перезапустить хелпер.', message_color)
		end
    end
end
